...
treatment_dirs = sorted(d for d in glob(path.join(root_dir, "*")) if path.isdir(d))
image_paths = sorted(str(f) for f in Path(treatment_dirs[0]).glob("*") if f.is_file())
...
def second_analyze(self, int_inputs, float_inputs, image_paths, mask_img):
    ...
    for file in image_paths[:2]:
        ...
        self.yes_button.place(relx = 0.35, rely = 0.55)
        self.no_button.place(relx = 0.5, rely = 0.55)
        self.batch_button.place(relx = 0.65, rely = 0.55)
...
def batch_analyze(self, int_inputs_first, float_inputs_first, int_inputs_all, float_inputs_all, scratch_orient):
    # for batch analysis of multiple image stacks
    try:
        inputs_raw_first = [int(input.get()) for input in int_inputs_first]+[float(input.get().replace(',', '.')) for input in float_inputs_first]
        inputs_first = dict(zip(('contr1', 'contr2', 'edge_er_iter','dil_iter', 'er_iter', 'dil', 'er', 'edge_er'), inputs_raw_first))

        inputs_raw_all = [int(input.get()) for input in int_inputs_all]+[float(input.get().replace(',', '.')) for input in float_inputs_all]
        entries_all = ('edge_contr1', 'edge_contr2', 'cell_contr1', 'cell_contr2', 'cell_fill', 
                'edge_er', 'edge_dil', 'cell_dil', 'cell_er', 'thresh')
        inputs_all = dict(zip(entries_all, inputs_raw_all))
    ...
    global treatment_dirs
    global mask_er 
    global mask_er_iter

    mode = self.mode.get()

    for treatment in treatment_dirs:

        print(f"CURRENTLY PROCESSING: {treatment}")

        image_paths = sorted(str(f) for f in Path(treatment).glob("*") if f.is_file())
        ...
        # image preprocessing
        mask_er = inputs_first['edge_er']
        mask_er_iter= inputs_first ['edge_er_iter']
        enhanced_img = enhance_contrast(first_img, inputs_first['contr1'], (inputs_first['contr2'], inputs_first['contr2']))
        blur_img = cv2.GaussianBlur(enhanced_img, (9, 9), 0)
        low_thresh = threshold_otsu(cv2.cvtColor(blur_img, cv2.COLOR_BGR2GRAY))
        canny_img = ~cv2.Canny(blur_img, low_thresh, 255)
        padded_img = cv2.copyMakeBorder(canny_img, 10, 10, 10, 10, cv2.BORDER_CONSTANT, None, 0)
        ...
        # detecting borders
        dilated_img = cv2.erode(padded_img, disk(inputs_first['dil']), iterations=inputs_first['dil_iter']) 
        eroded_img = cv2.dilate(dilated_img, disk(inputs_first['er']), iterations=inputs_first['er_iter'])
        contours, hierarchy = cv2.findContours(eroded_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) 
        sorted_contours = sorted(contours, key=cv2.contourArea, reverse= True) 
        blank_img = make_image(padded_img) 
        edges_img = cv2.drawContours(image=blank_img, contours=tuple(sorted_contours[:1]), contourIdx=-1, color=(255, 255, 255), 
                                    thickness= -1, lineType=cv2.LINE_AA) 
        mask = cv2.cvtColor(cv2.dilate(edges_img, disk(mask_er), iterations=mask_er_iter), cv2.COLOR_RGB2GRAY)
        
        ### analyzes all images in the folder
        hour = 0
        totals = []
        cell_fill = inputs_all['cell_fill']
        if cell_fill%2 == 0:
            cell_fill += 1   
        ...
        for file in image_paths:
            # image preprocessing for edges
            img = cv2.imread(file)
            enhanced_edges = enhance_contrast(img, inputs_all['edge_contr1'], (inputs_all['edge_contr2'], inputs_all['edge_contr2']))
            blur_edges = cv2.GaussianBlur(enhanced_edges, (9, 9), 0)
            canny_edges = ~cv2.Canny(blur_edges, threshold_otsu(cv2.cvtColor(blur_edges, cv2.COLOR_BGR2GRAY)), 255)
            padded_edges = cv2.copyMakeBorder(canny_edges, 10, 10, 10, 10, cv2.BORDER_CONSTANT, None, 0)
            overlay_edges = cv2.addWeighted(src1=padded_edges,alpha=0.5,src2=mask,beta=0.5,gamma=0)
            binary_edges = cv2.threshold(overlay_edges, 250, 255, cv2.THRESH_BINARY)[1]

            # detecting borders
            dilate_edges = cv2.erode(binary_edges, disk(inputs_all['edge_dil']), iterations=1)
            erode_edges = cv2.dilate(dilate_edges, disk(inputs_all['edge_er']), iterations=1)
            contours, hierarchy = cv2.findContours(erode_edges, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
            sorted_contours= sorted(contours, key=cv2.contourArea, reverse= True)
            ind = 0
            range_contours = range(len(sorted_contours))
            for n in range_contours[1:]:
                area = cv2.contourArea(sorted_contours[n])
                if area > 100:
                    if area/cv2.contourArea(sorted_contours[n-1]) < inputs_all['thresh']:
                            ind = n
                            break
                else:
                    ind = n
            ...
            edges_img = cv2.drawContours(image=blank, contours=tuple(sorted_contours[:ind]), contourIdx=-1, 
                                        color=(255, 255, 255), thickness= -1, lineType=cv2.LINE_AA)
            mask = cv2.cvtColor(cv2.dilate(edges_img, disk(mask_er), iterations=mask_er_iter), cv2.COLOR_BGR2GRAY)
            gray_edges_img = cv2.cvtColor(edges_img, cv2.COLOR_BGR2GRAY)

            if method == 'area':
                # image preprocessing for cells
                enhanced_cells = enhance_contrast(img, inputs_all['cell_contr1'], (inputs_all['cell_contr2'], inputs_all['cell_contr2']))
                blur_cells = cv2.GaussianBlur(enhanced_cells, (9, 9), 0)
                canny_cells = ~cv2.Canny(blur_cells, threshold_otsu(cv2.cvtColor(blur_cells, cv2.COLOR_BGR2GRAY)), 255)
                padded_cells = cv2.copyMakeBorder(canny_cells, 10, 10, 10, 10, cv2.BORDER_CONSTANT, None, 0)
                dilate_cells = cv2.erode(padded_cells, disk(inputs_all['cell_dil']), iterations=1)
                fill_cells = cv2.medianBlur(dilate_cells, inputs_all['cell_fill'])
                erode_cells = cv2.dilate(fill_cells, disk(inputs_all['cell_er']), iterations=1)
    ...
    self.end_label = ctk.CTkLabel(self, height=100, width=200, text='Batch processing is completed', corner_radius=100,
                            bg_color='#2d91e0', text_color='white')
    self.end_label.place(relx = 0.25, rely = 0.4)
    self.end_button = ctk.CTkButton(self, text='OK', fg_color='#104E8B', text_color='white', bg_color='#2d91e0',
                                command=lambda: app.destroy())
...
class App(ctk.CTk):
    def __init__(self, *args, **kwargs): 
        ...
        # storing first frame for global access
        self.first_frame = self.frames[First]
        self.show_frame(First)
...
class Second(ctk.CTkFrame):
    def __init__(self, parent, controller):
        super().__init__(parent)   # correct frame init
        ...
        self.batch_button = ctk.CTkButton(self, width = 50, text='Batch', fg_color='#104E8B', text_color='white', bg_color='#2d91e0',
                                        command=lambda: batch_analyze(self, self.controller.first_frame.int_entries, self.controller.first_frame.fl_entries, 
                                                                      self.int_entries, self.fl_entries, self.scratch_orient.get()))

        self.no_button = ctk.CTkButton(self, width = 50, text='No', fg_color='#104E8B', text_color='white', bg_color='#2d91e0',
                            command=lambda: widget_destroy(self.no_button, self.yes_button, self.batch_button, self.end_label))
